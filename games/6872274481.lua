...
run(function()
	...
	local function safeGetProto(fn, idx)
		if type(debug) ~= 'table' or type(debug.getproto) ~= 'function' or type(fn) ~= 'function' then
			return fn
		end

		local ok, result = pcall(debug.getproto, fn, idx)
