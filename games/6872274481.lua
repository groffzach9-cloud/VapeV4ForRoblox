...
local isnetworkowner = identifyexecutor and table.find({'AWP', 'Nihon', 'Delta'}, ({identifyexecutor()})[1]) and isnetworkowner or function()
	return true
end
...
run(function()
	...
	-- Game Remotes
	local serverEvents = repStorage:FindFirstChild('ServerEvents')
	local LookAngleRemote = serverEvents and serverEvents:FindFirstChild('LookAngle')
	local ShootRemote = serverEvents and serverEvents:FindFirstChild('Shoot')
	
	-- ==================== DESYNC MODULE ====================
	local Desync
	local DesyncMode, DesyncStrength, DesyncSpinSpeed
	local DesyncPitchDesync, DesyncPitchStrength, DesyncLBYBreaker
	local DesyncBodyRoll, DesyncVelDesync, DesyncVelStrength
	
	-- State
	local desyncModeValue = 'Jitter'
	local desyncStrengthValue = 60
	local desyncSpinSpeedValue = 90
	local desyncPitchDesyncValue = false
	local desyncPitchStrengthValue = 15
	local desyncLBYValue = false
	local desyncBodyRollValue = 0
		...
		Function = function(callback)
			if callback then
				if not isnetworkowner() then
					vape:CreateNotification('Desync', 'Network ownership required.', 5, 'alert')
					Desync:Toggle()
					return
				end
				...
				local realYaw = 0
				local fakeYaw = 0
				local sendTimer = tick()
				
				Desync:Clean(runService.PreSimulation:Connect(function(dt)
					...
					realYaw = math.atan2(-lookVec.X, lookVec.Z)
					
					local strength = desyncStrengthValue
					fakeYaw = realYaw
					
					if desyncModeValue == 'Spin' then
						spinAngle = (spinAngle + desyncSpinSpeedValue * dt) % 360
						fakeYaw = math.rad(spinAngle)
					elseif desyncModeValue == 'Jitter' then
						jitterSide = -jitterSide
						fakeYaw = realYaw + math.rad(strength * jitterSide)
					elseif desyncModeValue == 'Random' then
						fakeYaw = realYaw + math.rad(math.random(-strength, strength))
					elseif desyncModeValue == 'Static' then
						fakeYaw = realYaw + math.rad(strength)
					end
					
					-- Apply to Waist joint
					local upperTorso = entitylib.character.Character:FindFirstChild('UpperTorso')
					if upperTorso then
						local waist = upperTorso:FindFirstChild('Waist')
						if waist and waist:IsA('Motor6D') then
							local pitch = math.asin(gameCamera.CFrame.LookVector.Y)
							local fakePitch = pitch
							if desyncPitchDesyncValue then
								fakePitch = pitch + math.rad(desyncPitchStrengthValue)
							end
							waist.C0 = CFrame.new(0, 0.25, 0) * CFrame.Angles(-fakePitch + realYaw + math.rad(desyncBodyRollValue), 0, 0)
						end
					end
					
					-- Apply to Root joint (LBY Breaker)
					local lowerTorso = entitylib.character.Character:FindFirstChild('LowerTorso')
					if lowerTorso then
						local rootJoint = lowerTorso:FindFirstChild('Root')
						if rootJoint and rootJoint:IsA('Motor6D') then
							local moving = hum.MoveDirection.Magnitude > 0.1
							if desyncLBYValue and moving then
								local moveAngle = math.atan2(hum.MoveDirection.X, hum.MoveDirection.Z)
								rootJoint.C0 = CFrame.new(0, 0, 0) * CFrame.Angles(0, moveAngle, 0)
							else
					...
					-- Send fake LookAngle
					if LookAngleRemote and tick() - sendTimer > 0.05 then
						sendTimer = tick()
						local lookDir = CFrame.Angles(0, fakeYaw, 0).LookVector
						pcall(function()
							LookAngleRemote:FireServer(lookDir * 10000)
						end)
					end
					
					-- Velocity desync
					if desyncVelDesyncValue then
						local velStr = desyncVelStrengthValue * 0.01
						root.Velocity = root.Velocity + Vector3.new(
							math.random() * velStr - velStr * 0.5,
							0,
							math.random() * velStr - velStr * 0.5
						)
					end
				...
				if entitylib.isAlive then
					...
					if upperTorso then
						local waist = upperTorso:FindFirstChild('Waist')
						if waist and waist:IsA('Motor6D') then
							waist.C0 = CFrame.new(0, 0.25, 0)
						end
					end
				end
			end
		end,
		Tooltip = 'Manipulates view angles to confuse other players'
	})
	...
	DesyncMode = Desync:CreateDropdown({
		...
		Function = function(val)
			desyncModeValue = val
			if DesyncSpinSpeed then
				DesyncSpinSpeed.Object.Visible = (val == 'Spin')
			end
		end
	})
	
	DesyncStrength = Desync:CreateSlider({
		Name = 'Strength',
		Min = 0,
		Max = 180,
		Default = 60,
		Suffix = '°',
		Function = function(val)
			desyncStrengthValue = val
		end
	})
	
	DesyncSpinSpeed = Desync:CreateSlider({
		Name = 'Spin Speed',
		Min = 10,
		Max = 720,
		Default = 90,
		...
		Suffix = '°/s',
		Function = function(val)
			desyncSpinSpeedValue = val
		end
	})
	
	DesyncPitchDesync = Desync:CreateToggle({
		Name = 'Pitch Desync',
		Default = false,
		Function = function(callback)
			desyncPitchDesyncValue = callback
			if DesyncPitchStrength then
				DesyncPitchStrength.Object.Visible = callback
			end
		end
	})
	
	DesyncPitchStrength = Desync:CreateSlider({
		Name = 'Pitch Strength',
		Min = 0,
		Max = 45,
		Default = 15,
		...
		Suffix = '°',
		Function = function(val)
			desyncPitchStrengthValue = val
		end
	})
	
	DesyncLBYBreaker = Desync:CreateToggle({
		Name = 'LBY Breaker',
		Default = false,
		Function = function(callback)
			desyncLBYValue = callback
		end
	})
	
	DesyncBodyRoll = Desync:CreateSlider({
		Name = 'Body Roll',
		Min = 0,
		Max = 30,
		Default = 0,
		Suffix = '°',
		Function = function(val)
			desyncBodyRollValue = val
		end
	})
	
	DesyncVelDesync = Desync:CreateToggle({
		Name = 'Velocity Desync',
		Default = false,
		Function = function(callback)
			desyncVelDesyncValue = callback
			if DesyncVelStrength then
				DesyncVelStrength.Object.Visible = callback
			end
		end
	})
	
	DesyncVelStrength = Desync:CreateSlider({
		Name = 'Vel Strength',
		Min = 1,
		Max = 50,
		Default = 10,
